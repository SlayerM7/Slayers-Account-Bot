    ...
    run: (client, message, args, db) => {
        ...
        collector.on("end", (collected, reason) => {
            ...
            collected.forEach((value) => {
                if (questions[counter] === "Enter username:")
                    username = value.content;
                if (questions[counter] === "Enter password:")
            ...
            if (username !== null) {
                if (db.has(`account_${username}`)) {
                    ...
                    db.set(`account_${username}`, {
                        password: password,
                        username: username,
                        id: getRandom_1.getRandom(40000000000000, 90000000000000),
                        createdAt: new Date(),
                    ...
                    db.save();
                    message.channel.send(`Registered your account! You can log in now by using the \`login\` command`);
                }
            }
        });
    },
